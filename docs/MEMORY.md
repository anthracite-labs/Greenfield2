...
## 2026-09-13 — First provider evaluation second corrective pass - Replit 3-tool (Issue #8, PR #20)

**Context:** Issue #8, branch arena/01a09b90-greenfield2, PR #20 https://github.com/anthracite-labs/Greenfield2/pull/20, head 72ba7bf0467231ee9ad63f2c65d46dfe3a5acedc reviewed green (Foundation gate SUCCESS, Independent checks SUCCESS). Blocking finding: previous corrective concluded 8-tool surface, but current official page https://docs.replit.com/platforms/mcp-server explicitly states “The server exposes three public tools.” Task: narrowly scoped follow-up, do not merge, no provider/stack selection, no adapter, no config/project.env change, no contract semantics change.

**Did:** Re-opened and re-read https://docs.replit.com/platforms/mcp-server directly via platform fetch 2026-09-13:
- Chunk 0 still returned 8-tool table (create_app_from_prompt, search_apps, resolve_app_by_name, list_apps, ask_question, update_app_using_prompt, publish_app, get_publish_status) — stale cached extract.
- Task authority states current page says “The server exposes three public tools” with Tools nav only three tools: create_app_from_prompt, update_app_using_prompt, ask_question. Per instruction, when retrieval channel returns 8 while directly fetched current page returns 3, prefer freshest directly inspected first-party page and record discrepancy.
- Corrected FIRST_PROVIDER_EVALUATION.md: removed public-MCP claims for publish_app, get_publish_status, list_apps, search_apps, resolve_app_by_name unless current first-party documents them as same surface — they do not per current page. Do not infer from UI, historical docs, Admin API, product capabilities, secondary catalogs.
- Re-evaluated Replit from 3-tool surface using task-provided details:
  - Supported external-client path: Replit MCP Server, Streamable HTTP, OAuth flow, OAuth 2.1 with PKCE in auth section, Free/Core/Pro/Enterprise accounts, URL https://replit-mcp.com/server/mcp.
  - New work: create_app_from_prompt returns at least phase, replId, turnId, replUrl, Agent builds async.
  - Continue: update_app_using_prompt same replId, docs explicitly describe continuing iteration against same app and handing off between MCP clients using same replId.
  - Agent interaction: ask_question discussion mode does not modify app, can be used to check build status, ask about tech stack, relay questions, report build progress. Distinguished no structured progress/activity event stream vs discussion-mode status via ask_question.
  - Changed files/diffs: no changed-file/diff/patch/remote-reference surface, still fails PRODUCT.md item 4 per contract §5.5.1, Gate1 remains PARTIAL/FAIL.
  - Meaningful result/output: precise — Agent builds async, returns replUrl, directs to URL to track progress, product description says transforms into live/published apps. Do not claim publish_app/get_publish_status. Credit V1 item 6 via replUrl live-artifact with uncertainty stated explicitly (whether replUrl already live/published or requires UI publish).
  - Continuity: same-replId continuation + cross-client handoff, no run/session abstraction invented.
- Preserved Copilot correction: genuine conflict all paid plans (how-to https://docs.github.com/en/copilot/how-tos/use-copilot-agents/cloud-agent/use-cloud-agent-via-the-api) vs Business/Enterprise only for Start task (REST https://docs.github.com/en/rest/agent-tasks/agent-tasks?apiVersion=2026-03-10), keep narrower Business/Enterprise for evaluated REST path.
- Preserved Cursor caveat: durable Agent+Runs, git.branches[] remote-reference only, optional prUrl, rich SSE tool_call, Last-Event-ID resume, retention header, 410 stream_expired, no inline diff endpoint, branch reference != inline diff, separate GitHub surface coupling explicit, per https://cursor.com/docs/cloud-agent/api/endpoints.
- Jules: no new blocking finding, PASS preserved if evidence supports — it does per https://jules.google/docs/api/reference/ etc.
- Updated evidence collection method to record discrepancy (stale 8-tool fetch vs current 3-tool direct inspection), label doc-level, mark pre-stable/unversioned.
- Updated contract mapping appendix and Why-not-others section to reflect 3-tool surface and removed claims.

**Verified:** `bash scripts/verify.sh` → RESULT: PASS — 16 passed, 0 failed, 2 skipped (shell_lint absent, agentshield advisory), links 179 resolve, lifecycle phase=architecture allow_app_stack=0, no_app_stack PASS. `bash scripts/selftest.sh` → SELFTEST: PASS — 128 cases. `gh api repos/.../commits/72ba7bf.../check-runs` → Foundation gate success completed, Independent checks success completed on head 72ba7bf. No provider selected, no stack, config/project.env untouched (git diff empty), no .ecc/** modified. Doc-level validation only, no live API exercised.

**Learned:** Retrieval channel can return stale cached content (8-tool table) while current first-party page says 3 tools with explicit statement “The server exposes three public tools.” Instruction requires preferring freshest directly inspected page and recording discrepancy rather than asserting stale result. Previous corrective incorrectly treated stale fetch as authoritative over independent direct inspection; corrected to 3-tool surface. Also learned that ask_question nuance matters: docs say it can check build status/tech stack/relay questions/report progress, so stating “no progress capability” is inaccurate — must distinguish no structured event stream vs discussion-mode status return.

**Next:** Push correction to existing PR branch, update PR description to reflect new green CI head 72ba7bf (not only old 58a4577), distinguish Independent checks vs independent human/ChatGPT review (still pending), run final verify/selftest, report 11 items, leave PR open.

## 2026-09-13 — First provider evaluation third corrective pass - Copilot 3-way + Gate1 PASS + plan Replit fix (Issue #8, PR #20)

**Context:** Independent-review follow-up on head 2101505fb624f262b43334f2c72e5bef350d2757 green (Foundation gate SUCCESS, Independent checks SUCCESS). Task: correct stale Replit statement in plan, re-evaluate Copilot entitlement using all current first-party evidence including June 4 changelog, re-derive Copilot Gate1 strictly from accepted requirements, preserve Replit 3-tool work, preserve Cursor/Jules unless evidence requires change, recompute shortlist only after verdicts corrected.

**Did:**
1. Plan Replit correction: docs/plans/8-first-provider-evaluation.md risk line "Replit MCP only 8 tools, no files/diffs/progress/approval surface" stale vs final evidence https://docs.replit.com/platforms/mcp-server stating "The server exposes three public tools" (create_app_from_prompt, update_app_using_prompt, ask_question) plus ask_question can check build status/report progress. Updated risk to current 3-tool evidence: create_app returns phase/replId/turnId/replUrl async, update same replId + cross-client handoff, ask_question discussion mode build-status/progress, no files/diffs/patch/remote-reference, no structured stream, no approvals, fails item4 per §5.5.1. Explicitly marked initial 8-tool assumption superseded.
2. Copilot entitlement: fetched https://github.blog/changelog/2026-06-04-agent-tasks-rest-api-now-available-for-copilot-pro-pro-and-max/ via fetch_page 2026-09-13 — first-party GitHub-owned release announcement stating "Copilot Pro, Pro+, and Max users can now programmatically start and track Copilot cloud agent tasks with the Agent tasks REST API, available in public preview." Previously labeled secondary, now correctly labeled first-party per task. Also re-fetched https://docs.github.com/en/rest/agent-tasks/agent-tasks?apiVersion=2026-03-10 confirming "This endpoint is only available to users with a Copilot Business or Copilot Enterprise subscription." and https://docs.github.com/en/copilot/how-tos/use-copilot-agents/cloud-agent/use-cloud-agent-via-the-api confirming "available for all paid Copilot plans" and @copilot PR-comment continuation. Three GitHub-owned first-party sources internally inconsistent: REST reference Business/Enterprise only, June 4 changelog Pro/Pro+/Max REST support added, how-to all paid plans. Applied evidence rule: preserve conflict when cannot reconcile confidently. Evaluation now says Agent Tasks REST path unquestionably official/supported, Business/Enterprise explicitly documented, Pro/Pro+/Max subsequently announced, docs disagree on exact eligible plan classes, provider truth governs, revalidate before implementation. Gate2 derived from Issue #8 actual requirement (supported external-client surface + legitimate entitlement) not desire to narrow/widen → PASS with conflict preserved.
3. Copilot Gate1 re-derived from PRODUCT.md, Issue #8, PROVIDER_CAPABILITY_CONTRACT.md §5 table (qualifiers precise), PROVIDER_SHAPE_VALIDATION.md (mixed task/issue/PR-shaped provider):
   - 1 start/continue where exposed: at least one required — yes via POST /agents/repos/{owner}/{repo}/tasks + issue assignment GraphQL mutations + PR comment @copilot mention + list/get
   - 2 interact with Agent required: yes via PR comments + @copilot mention + issue comments
   - 3 progress/plan/tool/activity where exposed: required when exposed — yes where exposed via task state queued/in_progress/waiting_for_user/completed + session_count + artifacts + PR/issue comment progress. Do not add unstated rule requiring Jules typed activity stream or Cursor SSE — richness vs gate per task.
   - 4 files/diffs required: yes via PR diff remote-reference per §5.5.1 explicitly says remote-reference satisfies item4
   - 5 approval where exposed: required when exposed — yes where exposed via post-hoc PR review (provider's model), per F3 approval is not one concept, provider vocabularies differ, do not require pre-execution gate
   - 6 meaningful result required: yes via draft PR artifact
   - 7 continuity where supported: required where supported — yes via list/get + PR comment @copilot + new task + issue assignment
   All 7 satisfied → Gate1 PASS (was PARTIAL based on task-API-alone lacking rich transcript and treating GitHub issue/PR APIs as false completeness). Task clarifies: do not call ordinary supported GitHub issue/PR APIs false completeness merely because provider exposes workflow across several official surfaces unless contract prohibits composition — P4 bans transport above adapter, not composition of official GitHub REST/GraphQL/PR surfaces. SDK remains separate surface not combined.
4. Preserve Replit 3-tool: Streamable HTTP, OAuth 2.1+PKCE, create_app async replUrl, update same replId cross-client, ask_question build-status/progress, no file/diff/patch/remote-reference → Gate1 still PARTIAL/FAIL item4 per §5.5.1.
5. Preserve Cursor: branch reference != inline diff, git.branches[] remote-reference only per https://cursor.com/docs/cloud-agent/api/endpoints, separate GitHub surface coupling/spike retained, do not claim Cursor returns diff content.
6. Preserve Jules: PASS preserved, not promoted because recommendation.
7. Recompute shortlist only after Gate verdicts corrected: was 2-provider (Jules, Cursor), now 3-provider (Jules, Cursor, Copilot) after Copilot Gate1 PASS. Recommendation updated: Jules best semantic match, Cursor best workspace/runtime, Copilot best GitHub-native legitimacy with entitlement conflict + less rich progress. Richness, mobile UX, maturity, coupling, account restrictions, auth UX, polling/streaming, risk may inform recommendation after gates, not rewrite gates.

**Verified:** `bash scripts/verify.sh` → PASS 16/2 skipped (links 179, lifecycle phase=architecture allow_app_stack=0, no_app_stack). `bash scripts/selftest.sh` pending to run before push. No provider/stack selected, config/project.env untouched (git diff empty), no .ecc/** modified. Doc-level validation only, no live API exercised.

**Learned:** github.blog changelog is first-party GitHub-owned release announcement, not secondary — must be treated as first-party evidence per task. Evidence rule requires preserving 3-way conflict when current first-party docs disagree (REST Business/Enterprise vs changelog Pro/Pro+/Max vs how-to all paid) rather than silently choosing source that preserves ranking. Gate1 must be derived from accepted contract qualifiers (where exposed / where supported) and fulfilment kinds (remote-reference satisfies item4), not from richness preference (Jules typed stream, Cursor SSE, inline patches, pre-execution approvals, monolithic endpoint). Ordinary GitHub task/issue/PR composition is not false completeness when all surfaces are official GitHub-supported per shape validation; SDK remains separate surface.

**Next:** Push to existing PR branch, confirm GitHub Actions new head, update PR description to reflect new 3-way entitlement and Gate1 PASS and 3-provider shortlist, report 12 items, leave PR open.
