...
Current Architecture work is the first-provider evaluation in [Issue #8](https://github.com/anthracite-labs/Greenfield2/issues/8). Issue #9 is complete: [ADR-0005](docs/decisions/0005-provider-contract-before-provider-selection.md) required the provider-neutral capability boundary to be defined and validated across at least three materially different provider shapes before any first-provider selection, and that precondition is now discharged. The accepted contract is recorded as [v1.1](docs/architecture/PROVIDER_CAPABILITY_CONTRACT.md), its structure is recorded in [ADR-0006](docs/decisions/0006-capability-manifest-and-three-layer-availability.md), and [four provider shapes were validated](docs/architecture/PROVIDER_SHAPE_VALIDATION.md), forcing fifteen revisions. No provider is selected by that work, and implementation stays locked. Jules, Cursor, Replit, GitHub Copilot, VibeFlow and Happier are evidence inputs only, and none is selected or privileged by having been tested. `ALLOW_APP_STACK=0` continues to block application implementation.